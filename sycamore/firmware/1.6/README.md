...
- Fixed a problem with the Transposition auxMode leading to a 1 semitone transposition by default. Configure a `transposeOffset` in settings.txt if Sycamore is offsetting sequences by a semitone even with 0v entered into the aux input. The 1.6 firmware comes with a -1 `transposeOffset`. With thanks to David Potter for reporting the problem.
...
Syntax for enabling the Rowan expander, Oak's LED brightness, and the transposition feature in settings.txt:
