...
while True:
    ...
    if not (clockSwitch.pressed or clockTriggerDebounced.fell or gc.clockBeforeGc == True):
        ...
        # Normalise transposition. Unfortuantely the ADC values being read out tend to give a small
        # offset to the transposition when in transpose mode, so this needs to be corrected.
        if auxModeTranspose:
            transpose = transpose + config.getTransposeOffset()
            # Only positive (or neutral) transposition is supported
            # Large transpose values (including with an offset) will
            # be clipped to dacLUTMax below.
            if transpose < 0:
                transpose = 0
